...
def encrypt_hmac_sha256_siv(k: bytes, m: bytes) -> tuple[bytes, bytes]:
    ...
    c = bytes(a ^ b for a, b in zip(k_x, m))  # Python's zip and list comprehension for XOR
...
def test_known_hash():
    compare_known_hash(
        ...
        bytes.fromhex("ab7e8499d21f80a6600b3b9ee349ac6d72c07e3359fe885a934ba7aa844429f8"),
        bytes.fromhex(
...
def test_known_hash2():
    compare_known_hash(
        b"anotherpassword",
        bytes.fromhex("202122232425262728292a2b2c2d2e2f303132333435363738393a3b3c3d3e3f"),
        bytes.fromhex("88a787415a2ecd79da0d1016a82a27c5c695c9a19b88b0aa1d35683280aa9a67"),
        bytes.fromhex("301d9dd1e96f20ce51083f67d3298fd37b97525de8324d5e12ed2d407d3d927b"),
        bytes.fromhex(
