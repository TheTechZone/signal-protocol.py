    ...
    for obj in objects:
        ...
        if inspect.isclass(obj[1]):
            ...
            methods = [
                ...
                if method
                not in (base_class_dict or class_dict[method] != base_class_dict.get(method))
                and method not in extra_excludes
            ]

            logger.debug(f"Methods of {obj[0]} (excluding inherited but including overridden):")
...
def create_dummy(symbol_dict):
    ...
    ast_nodes = []
    tag = ast.Expr(value=ast.Constant("[AUTOGENERATED] -- TODO: adapt (if needed) and remove me"))

    # Function to create a class AST node
    def create_class_ast(class_name):
        class_def = ast.ClassDef(name=class_name, bases=[], keywords=[], body=[], decorator_list=[])
